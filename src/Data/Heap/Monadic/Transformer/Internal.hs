...
(<>!) :: Semigroup a => a -> NodeT a m b -> NodeT a m b
x <>! (y :< ys) = (x <> y) :< ys
{-# INLINE (<>!) #-}

(<>*) :: Semigroup a => a -> HeapT a m b -> HeapT a m b
_ <>* Empty = Empty
x <>* Tree xs = Tree (x <>! xs)
{-# INLINE (<>*) #-}

instance (Ord a, Group a, Functor m) => Semigroup (NodeT a m b) where
  (x :< xs) <> (y :< ys)
    | x <= y    = x :< (fmap.fmap) ((:-) ((inv x <> y) :< ys)) xs
    | otherwise = y :< (fmap.fmap) ((:-) ((inv y <> x) :< xs)) ys
  {-# INLINE (<>) #-}

instance (Ord a, Group a, Functor m) => Semigroup (HeapT a m b) where
    Empty <> ys = ys
    xs <> Empty = xs
